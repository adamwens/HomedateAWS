...
def do_send(memberPhone, memberName, targetPhone, targetName, receiveTimeStr, dateTimeStr, dateLink, smsType):
    phone = ''
    message = ''
    if smsType == 0:
        phone = memberPhone
        message = OTP_MSG_MEMBER.format(targetName, dateTimeStr, dateLink)
    elif smsType == 1:
        phone = targetPhone
        message = OTP_MSG_TARGET.format(memberName, dateTimeStr, dateLink)
    else:
        print('do_send get weird sms type.')
        return 1
    print(message)
    
    if not phone or not message:
        print('do_send get null phone or message.')
        return 1
...
def lambda_handler(event, context):
    try:
        ...
        if smsType == 0:
            if not check_string(memberPhone) or not check_string(targetName) or not check_string(dateLink) or not dateTimeStr:
                context = {
                    'statusCode': 400,
                    'headers': {
            ...
            if not check_string(memberName) or not check_string(targetPhone) or not check_string(dateLink) or not dateTimeStr:
                context = {
                    'statusCode': 400,
                    'headers': {
                        'Access-Control-Allow-Headers': 'Content-Type',
    ...
    try:
        # sendRet = do_send(memberPhone, memberName, targetPhone, targetName, receiveTimeStr, dateTimeStr, dateLink, smsType)
        sendRet = do_send(memberPhone, memberName, targetPhone, targetName, '', dateTimeStr, dateLink, smsType)
    except:
        pass
